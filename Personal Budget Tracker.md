...
\- Pseudocode: The pseudocode represents the same logic but in a structured, code-like format that is easier to translate into C++. It starts by displaying the program title and prompting the user for inputs such as name, period, and the number of income sources. A loop is used to collect each income amount, with a check to reject negative values. The same structure is applied to expenses. After gathering all inputs, the pseudocode calculates the balance and uses conditional statements to classify the budget status. It then outputs the report and saves it to a file. The pseudocode is essentially the bridge between the plain-language algorithm and the actual C++ implementation, showing the logical flow in programming terms without strict syntax. See 'pseudocode.md'
